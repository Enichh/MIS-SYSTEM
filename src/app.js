import { Employee, Project, Task, WorkType, generateId } from '../shared/models.js';
import {
  openDatabase,
  createEmployee,
  getEmployee,
...
function setupEventListeners() {
  ...
  document.getElementById('fab-button').addEventListener('click', openChatModal);
  document.getElementById('chat-modal-close').addEventListener('click', closeChatModal);
  document.getElementById('chat-modal-overlay').addEventListener('click', handleChatModalOverlayClick);
...
async function openEditModal(entity, id) {
  ...
  if (!data) {
    showNotification(`${entity.charAt(0).toUpperCase() + entity.slice(1)} not found`, 'error');
    return;
  }
...
async function handleFormSubmit(event) {
  ...
  try {
    ...
    closeModal();
    showNotification(`${entity.charAt(0).toUpperCase() + entity.slice(1)} saved successfully`, 'success');
...
async function handleDelete(entity, id) {
  ...
  try {
    ...
    showNotification(`${entity.charAt(0).toUpperCase() + entity.slice(1)} deleted successfully`, 'success');
    await refreshCurrentSection();
  } catch (error) {
    showNotification(error.message, 'error');
  }
...
async function handleRemoveFromTask(employeeId, taskId) {
  if (!confirm('Are you sure you want to remove this employee from the task?')) {
    return;
  }
